...
@Slf4j
public class LguV2SmsStrategy implements SmsStrategy {
    
    private final SmsRepository smsRepository;
    
    @Override
    public MessageResult send(SmsMessage message) {
        log.info("Storing SMS for LGU V2 agent pickup: {}", message.getMessageId());
        
        try {
            message.setChannel(MessageChannel.LGU_V2);
            message.setStatus(MessageStatus.PENDING);
            smsRepository.save(message);
            
            log.info("SMS stored successfully for LGU V2 agent pickup: {}", message.getMessageId());
            return MessageResult.success(message.getMessageId(), message.getMessageId());
            
        } catch (Exception e) {
            log.error("Failed to store SMS for LGU V2: {}", message.getMessageId(), e);
    ...
    public boolean isHealthy() {
        try {
            ...
            log.warn("LGU V2 SMS strategy health check failed", e);
            return false;
        }
    }
}